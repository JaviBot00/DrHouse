...
public class DatabaseAccess implements DataRepository {
    ...
    public List<Medicamento> listMedicamentosXTratamiento(String dni, String id_tratamiento) throws SQLException, ClassNotFoundException {
        List<Medicamento> results = new ArrayList<>();
        String sql = "SELECT m.codigo, m.nombre, m.posologia " +
            "FROM " + TABLE_MEDICAMENTO_TRATAMIENTO + " mt " +
            "JOIN " + TABLE_MEDICAMENTO + " m ON m.codigo = mt.codigo_medicamento " +
            "JOIN " + TABLE_TRATAMIENTO + " t ON t.id_tratamiento = mt.id_tratamiento " +
            "WHERE t.dni_paciente = ? AND t.id_tratamiento = ?";
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, dni);
    ...
    private void insertarMedicamentoTratamiento(Connection conn, Tratamiento t, Medicamento m) throws SQLException {
        String sql = "INSERT IGNORE INTO " + TABLE_MEDICAMENTO_TRATAMIENTO + " (id_tratamiento, codigo_medicamento) VALUES (?, ?)";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, t.getIdTratamiento());
            ps.setString(2, m.getIdMedicamento());
            ps.executeUpdate();
        }
    }
